  ...
  try {
    ...
    if (registry.include?.length) {
      throw new RegistryValidationError(
        `Registry catalog "${name}" uses "include", but consumer registry endpoints must serve a resolved registry catalog. Run "npx love-ui build" and serve the built registry.json, or use loadRegistry() in a dynamic route.`,
        {
          context: {
            registry: name,
          ...
          suggestion:
            "Serve a flattened registry.json for CLI consumers. Source registry.json files with include are supported by love-ui build and loadRegistry().",
        }
      )
    }
    ...
    throw new RegistryParseError(name, error, {
      ...
      suggestion:
        "The registry catalog may be corrupted or have an invalid format. Please make sure it returns a valid registry.json object. See https://www.loveui.dev/schema/registry.json.",
    })
  }
}
...
export async function getRegistryBaseColor(baseColor: string) {
  try {
    const [result] = await fetchRegistry([
      ...
      return fetchRegistry([`colors-${baseColor}.json`]).catch(
        async (fallbackError) => {
          if (!(fallbackError instanceof RegistryNotFoundError)) {
            throw fallbackError
          }

          return fetchRegistry(["colors-zinc.json"])
        }
      )
    })
