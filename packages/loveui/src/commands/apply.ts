      ...
      if (!preset) {
        ...
        await promptToOpenPresetBuilder({
          ...
          followUp: `Then run ${highlighter.info(
            "love-ui apply --preset <preset>"
          )} with the preset code or preset URL from loveui.dev.`,
          prompt: !options.yes,
        })
  ...
  if (value === true) {
    throw new ApplyOnlyError(
      [
        ...
        "Example: love-ui apply <preset> --only theme,font.",
      ].join("\n")
    )
  }
...
export function validateApplyOnlyPreset(options: {
  ...
  throw new ApplyOnlyError(
    [
      ...
      "Use: love-ui apply <preset> --only theme,font.",
    ].join("\n")
  )
}
  ...
  if (!knownPresetNames.includes(preset)) {
    logger.error(
      `Invalid preset: ${highlighter.info(
        preset
      )}.\nUse one of the available presets: ${knownPresetNames.join(", ")} \nor build your own at ${highlighter.info(`${LOVEUI_URL}/create`)}`
    )
    logger.break()
    process.exit(1)
  }
...
export function resolveApplyInitUrl(
  ...
  if (isUrl(preset)) {
    ...
    if (url.pathname === "/init" && preset.startsWith(LOVEUI_URL)) {
...
function getInitCommand(preset?: string) {
  ...
  return `love-ui init --preset ${quoteShellArg(preset)}`
}
