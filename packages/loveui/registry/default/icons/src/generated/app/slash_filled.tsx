/** Auto-generated - Do not edit */
'use client';
import React from 'react';

export interface SlashFilledProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  strokeWidth?: number;
}

export const SlashFilled = React.forwardRef<SVGSVGElement, SlashFilledProps>(
  ({ size = 24, className = '', strokeWidth = 1, ...props }, ref) => (
    <svg 
      ref={ref}
      width={size}
      height={size} 
      viewBox="0 0 24 24"
      fill="currentColor"
      stroke="none"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...(strokeWidth !== undefined ? { strokeWidth } : {})}
      {...props}
    >
      <path d="M19.53 5.53L5.53 19.53C5.51 19.55 5.5 19.56 5.48 19.57C5.1 19.25 4.75 18.9 4.43 18.52C2.91 16.77 2 14.49 2 12C2 6.48 6.48 2 12 2C14.49 2 16.77 2.91 18.52 4.43C18.9 4.75 19.25 5.1 19.57 5.48C19.56 5.5 19.55 5.51 19.53 5.53Z" fill="currentColor"/>
<path d="M22 12.0001C22 17.4901 17.49 22.0001 12 22.0001C10.5 22.0001 9.07998 21.6701 7.79998 21.0701C7.17998 20.7801 7.05998 19.9501 7.53998 19.4601L19.46 7.5401C19.94 7.0601 20.78 7.1801 21.07 7.8001C21.67 9.0701 22 10.5001 22 12.0001Z" fill="currentColor"/>
<path d="M21.77 2.22988C21.47 1.92988 20.98 1.92988 20.68 2.22988L2.23 20.6899C1.93 20.9899 1.93 21.4799 2.23 21.7799C2.38 21.9199 2.57 21.9999 2.77 21.9999C2.97 21.9999 3.16 21.9199 3.31 21.7699L21.77 3.30988C22.08 3.00988 22.08 2.52988 21.77 2.22988Z" fill="currentColor"/>
    </svg>
  )
);
SlashFilled.displayName = "SlashFilled";
export const SlashFilledMetadata = { 
  id: "slash_filled", 
  baseId: "slash", 
  variant: "filled", 
  name: "Slash", 
  category: "app", 
  tags: [], 
  viewBox: "0 0 24 24" 
} as const;

export default SlashFilled;
