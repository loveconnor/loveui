/** Auto-generated - Do not edit */
'use client';
import React from 'react';

export interface VideoVerticalFilledProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  strokeWidth?: number;
}

export const VideoVerticalFilled = React.forwardRef<SVGSVGElement, VideoVerticalFilledProps>(
  ({ size = 24, className = '', strokeWidth = 1, ...props }, ref) => (
    <svg 
      ref={ref}
      width={size}
      height={size} 
      viewBox="0 0 24 24"
      fill="currentColor"
      stroke="none"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...(strokeWidth !== undefined ? { strokeWidth } : {})}
      {...props}
    >
      <path d="M6.36 7.78003H2V11.25H6.36V7.78003Z" fill="currentColor"/>
<path d="M6.3599 6.28988V2.12988C4.0799 2.54988 2.5899 4.01988 2.1499 6.27988H6.3299C6.3399 6.27988 6.3499 6.28988 6.3599 6.28988Z" fill="currentColor"/>
<path d="M6.36 16.31V12.75H2V16.28H6.24C6.28 16.28 6.32 16.3 6.36 16.31Z" fill="currentColor"/>
<path d="M21.8501 6.27993C21.4201 4.08993 20.0201 2.64993 17.8601 2.17993V6.27993H21.8501Z" fill="currentColor"/>
<path d="M16.3601 11.25V2.01C16.3101 2 16.2501 2 16.1901 2H7.86011V11.25H16.3601Z" fill="currentColor"/>
<path d="M22.0001 12.75H17.8601V16.28H22.0001V12.75Z" fill="currentColor"/>
<path d="M17.8601 21.82C20.0001 21.35 21.4001 19.93 21.8401 17.78H17.8601V21.82Z" fill="currentColor"/>
<path d="M6.23991 17.78H2.15991C2.61991 20 4.09991 21.45 6.35991 21.87V17.76C6.31991 17.77 6.27991 17.78 6.23991 17.78Z" fill="currentColor"/>
<path d="M22.0001 7.78003H17.8601V11.25H22.0001V7.78003Z" fill="currentColor"/>
<path d="M7.86011 12.75V22H16.1901C16.2501 22 16.3101 22 16.3601 21.99V12.75H7.86011Z" fill="currentColor"/>
    </svg>
  )
);
VideoVerticalFilled.displayName = "VideoVerticalFilled";
export const VideoVerticalFilledMetadata = { 
  id: "video-vertical_filled", 
  baseId: "video-vertical", 
  variant: "filled", 
  name: "Video Vertical", 
  category: "multimedia", 
  tags: [], 
  viewBox: "0 0 24 24" 
} as const;

export default VideoVerticalFilled;
