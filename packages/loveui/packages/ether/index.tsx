...
interface LiquidEtherWebGL {
  output?: {
    ...
    output?: { material?: THREE.Material | THREE.Material[] | null };
  };
  autoDriver?: {
    enabled: boolean;
    speed: number;
...
export default function LiquidEther({
  ...
  const createPaletteTexture = useCallback((stops?: string[]) => {
    ...
    class MouseClass {
      ...
      onDocumentTouchStart(event: TouchEvent) {
        if (event.touches.length === 1) {
          ...
          if (!touch) return;
          if (this.onInteract) this.onInteract();
          this.setCoords(touch.pageX, touch.pageY);
          this.hasUserControl = true;
        }
      }
      ...
      update() {
        ...
        if (!this.takeoverActive && this.coords_old.x === 0 && this.coords_old.y === 0) {
          this.diff.set(0, 0);
        }
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        material.uniforms.force.value.set(forceX, forceY);
        material.uniforms.center.value.set(centerX, centerY);
        material.uniforms.scale.value.set(cursorSize, cursorSize);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        let fbo_in: { texture: THREE.Texture } | null = null;
        let fbo_out: { texture: THREE.Texture } | null = null;
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        let p_in: { texture: THREE.Texture } | null = null;
        let p_out: { texture: THREE.Texture } | null = null;
        const iter = iterations ?? 0;
        for (let i = 0; i < iter; i++) {
          if (i % 2 === 0) {
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this as any, {
          enabled: false,
          // auto animation is disabled by default; toggled via props
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      if (!autoDemo) {
        webgl.autoDriver.forceStop()
        if (webgl.autoDriver.mouse) {
          webgl.autoDriver.mouse.hasUserControl = true
          webgl.autoDriver.mouse.isAutoActive = false
        }
  ...
  useEffect(() => {
    ...
    const shaderMaterial = material as THREE.RawShaderMaterial
    if (!shaderMaterial.uniforms?.palette) return
    ...
    shaderMaterial.uniforms.palette.value = newTexture
  }, [colors, createPaletteTexture])
