...
import { BUILTIN_REGISTRIES, LOVEUI_URL } from "@/src/registry/constants"
        ...
        if (options.template === "laravel" && !hasPackageJson) {
          ...
          logger.log(
            `  Please create a new app with ${highlighter.info(
              "laravel new --react"
            )} first then run ${highlighter.info("love-ui init")}.`
          )
          logger.log(
            `  See ${highlighter.info(
              `${LOVEUI_URL}/docs/installation/laravel`
      ...
      if (options.preset !== undefined) {
        ...
        if (typeof presetArg === "string") {
          ...
          if (isUrl(presetArg)) {
            ...
            if (url.pathname === "/init" && presetArg.startsWith(LOVEUI_URL)) {
