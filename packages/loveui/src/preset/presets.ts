...
import { REGISTRY_URL, LOVEUI_URL } from "@/src/registry/constants"
...
export function resolveCreateUrl(
  ...
  const url = new URL(`${LOVEUI_URL}/create`)
...
export function resolveInitUrl(
  ...
  return `${LOVEUI_URL}/init?${params.toString()}`
}
...
function isLoveUIInitUrl(url: string) {
  try {
    return new URL(url).pathname === "/init" && url.startsWith(LOVEUI_URL)
  } catch {
    return false
  }
}
