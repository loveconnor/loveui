...
describe("configWithDefaults", () => {
  ...
  it("should preserve user registries when merging", () => {
    ...
    expect(result.registries?.["@love-ui"]).toBe(BUILTIN_REGISTRIES["@love-ui"])
  })
  ...
  it("should override built-in registries if user provides same key", () => {
    const config = createConfig({
      registries: {
        "@love-ui": "http://custom-loveui.com/{name}",
      },
    })

    const result = configWithDefaults(config)

    // User's @love-ui should override the built-in one
    expect(result.registries?.["@love-ui"]).toBe(
      "http://custom-loveui.com/{name}"
    )
  })
