...
import { runEject, LOVEUI_TAILWIND_IMPORT } from "./eject"
...
describe("runEject", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await mkdtemp(path.join(os.tmpdir(), "love-ui-eject-"))
  })
  ...
  it("inlines love-ui/tailwind.css and removes the dependency", async () => {
    ...
    const loveUiCss = await readFile(
      path.join(process.cwd(), "src/tailwind.css"),
      "utf8"
    )

    expect(output).not.toMatch(LOVEUI_TAILWIND_IMPORT)
    expect(output).toContain('@import "tailwindcss";')
    expect(output).toContain('@import "tw-animate-css";')
    expect(output).toContain("/* ejected from love-ui@4.8.3 */")
    expect(output).toContain(loveUiCss.trim())
    expect(output).toContain("@layer base")
    expect(output).toContain("@apply bg-background text-foreground;")
    expect(execa).toHaveBeenCalledWith("pnpm", ["remove", "love-ui"], {
      cwd: tempDir,
    })
  })

  it("removes love-ui from devDependencies", async () => {
    await setupProject(baseCss, {
      name: "test-app",
      devDependencies: {
        "love-ui": "^4.8.3",
  ...
  it("skips dependency removal when love-ui is not in package.json", async () => {
    await setupProject(baseCss, {
      name: "test-app",
      dependencies: {},
    })
  ...
  it("removes love-ui from package.json for deno", async () => {
    ...
    expect(packageJson.dependencies?.["love-ui"]).toBeUndefined()
    expect(execa).not.toHaveBeenCalled()
  })

  it("exits when love-ui/tailwind.css is not imported", async () => {
