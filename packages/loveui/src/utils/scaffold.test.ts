...
describe("defaultScaffold", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    delete process.env.LOVEUI_TEMPLATE_DIR
    delete process.env.LOVEUI_GITHUB_URL
    mockExit = vi
      .spyOn(process, "exit")
  ...
  it("should clone the repo with sparse checkout", async () => {
    ...
    expect(vi.mocked(execa)).toHaveBeenCalledWith("git", [
      ...
      "--filter=blob:none",
      "--sparse",
      "https://github.com/loveconnor/loveui.git",
      expect.stringContaining("love-ui-template-"),
    ])
  ...
  it("should use local templates when LOVEUI_TEMPLATE_DIR is set", async () => {
    process.env.LOVEUI_TEMPLATE_DIR = "/local/templates"
