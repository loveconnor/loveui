...
const LOVEUI_CLI_COMMAND = "love-ui@latest"

export async function loveUiCli(command: string) {
  const packageRunner = await getPackageRunner(process.cwd())
  return `${packageRunner} ${LOVEUI_CLI_COMMAND} ${command}`
}
...
export function formatSearchResultsWithPagination(
  ...
  const formattedItems = results.items.map((item) => {
    ...
    parts.push(`\n  Add command: \`npx love-ui@latest add ${item.addCommandArgument}\``)
