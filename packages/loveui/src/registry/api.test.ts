...
const server = setupServer(
  ...
  http.get(`${REGISTRY_URL}/registries.json`, () => {
    return HttpResponse.json([
      {
        name: "@love-ui",
        url: "https://www.loveui.dev/r/styles/{style}/{name}.json",
        description: "The official loveui registry.",
      },
      {
        name: "@example",
...
describe("getRegistryItem", () => {
  it("should read and parse a valid local JSON file", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tmpdir(), "loveui-test-"))
  ...
  it("should handle tilde (~) home directory paths", async () => {
    ...
    const tempFile = path.join(homeDir, "loveui-test-tilde.json")
    ...
    try {
      ...
      const tildeePath = "~/loveui-test-tilde.json"
...
describe("getRegistry", () => {
  ...
  it("should throw RegistryParseError with proper context", async () => {
    ...
    try {
      ...
      if (error instanceof RegistryParseError) {
        ...
        expect(error.suggestion).toContain(
          "https://www.loveui.dev/schema/registry.json"
        )
  ...
  it("should distinguish between URL and registry name", async () => {
    ...
    const registryName = "@love-ui"
    const registryUrl = "https://www.loveui.dev/registry.json"

    // Mock for URL
    server.use(
      http.get(registryUrl, () => {
        return HttpResponse.json({
          name: "loveui-from-url",
    ...
    const mockConfig = {
      ...
      registries: {
        "@love-ui": {
          url: "https://www.loveui.dev/{name}.json",
        },
      },
    } as any
    ...
    expect(urlResult.name).toBe("loveui-from-url")
...
describe("getRegistriesConfig", () => {
  ...
  it("should handle complex registry configurations", async () => {
    ...
    try {
      ...
      expect(result.registries?.["@love-ui"]).toBe(
        "https://www.loveui.dev/r/styles/{style}/{name}.json"
      )
