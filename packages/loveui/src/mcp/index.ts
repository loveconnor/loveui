  ...
  return {
    tools: [
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            registries: z
              .array(z.string())
              .describe(
                "Array of registry names to search (e.g., ['@love-ui', '@acme'])"
              ),
            limit: z
              .number()
              .optional()
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                "Array of item names with registry prefix (e.g., ['@love-ui/button', '@love-ui/card'])"
              ),
          })
        ),
      },
      ...
      },
      {
        ...
        description:
          "Get the Love UI CLI add command for specific items in a registry. This is useful for adding one or more components to your project.",
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                "Array of items to get the add command for prefixed with the registry name (e.g., ['@love-ui/button', '@love-ui/card'])"
              ),
          })
        ),
      },
          ...
          content: [
            {
              ...
              text: dedent`The following registries are configured in the current project:
                ...
                \`${await loveUiCli("view @name-of-registry")}\`

                For example: \`${await loveUiCli(
                  "view @love-ui"
                )}\` or \`${await loveUiCli(
                  "view @love-ui @acme"
            ...
            content: [
              {
                ...
                Make sure the item names are correct and include the registry prefix (e.g., @love-ui/button).`,
              },
            ],
          }
        }
      ...
      case "get_add_command_for_items": {
        ...
        return {
          content: [
            {
              type: "text",
              text: await loveUiCli(`add ${args.items.join(" ")}`),
            },
          ],
        }
      }
