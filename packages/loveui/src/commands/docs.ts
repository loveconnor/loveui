...
import { getLoveUIRegistryIndex } from "@/src/registry/api"
...
const LOVEUI_BASE_URL = "https://www.loveui.dev"
...
export const docs = new Command()
  ...
  .action(async (components, opts) => {
    try {
      ...
      const index = await getLoveUIRegistryIndex()
        ...
        if (!item) {
          logger.error(
            `Component ${highlighter.info(
              component
            )} not found in the Love UI registry.`
          )
          process.exit(1)
        }
...
function normalizeLinks(links: Record<string, string>) {
  return Object.fromEntries(
    Object.entries(links).map(([key, value]) => [
      key,
      value.startsWith(LOVEUI_BASE_URL)
        ? `${LOVEUI_URL}${value.slice(LOVEUI_BASE_URL.length)}`
        : value,
    ])
  )
}
