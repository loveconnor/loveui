...
describe("validateRegistryConfig", () => {
  ...
  describe("built-in registries", () => {
    it("should not throw for @love-ui since it's now a built-in registry", () => {
      expect(() => {
        validateRegistryConfig("@love-ui", {
    ...
    it("should not throw for similar but different registry names", () => {
      expect(() => {
        validateRegistryConfig("@love-ui-ui", {
          url: "https://example.com/{name}",
        })
      }).not.toThrow()

      expect(() => {
        validateRegistryConfig("@myloveui", {
          url: "https://example.com/{name}",
        })
      }).not.toThrow()
    })
