...
export const getPackage = async (packageName: string) => {
  ...
  const loveUIDependencies = Object.keys(packageJson.dependencies || {}).filter(
    (dep) => dep.startsWith("@repo") && dep !== "@repo/shadcn-ui"
  );

  const dependencies = Object.keys(packageJson.dependencies || {}).filter(
    (dep) =>
      !["react", "react-dom", "@repo/shadcn-ui", ...loveUIDependencies].includes(
        dep
      )
  );
  ...
  for (const file of tsxFiles) {
    ...
    files.push({
      ...
      path: file.name,
      content,
      target: `components/love-ui/${packageName}/${file.name}`,
    });
  }
  ...
  for (const dep of loveUIDependencies) {
    const pkg = dep.replace("@repo/", "");

    registryDependencies.push(`https://www.loveui.dev/r/${pkg}.json`);
  }
  ...
  const response: RegistryItem = {
    ...
    author: "Connor Love <hello@loveconnor.com>",
    dependencies,
    devDependencies,
