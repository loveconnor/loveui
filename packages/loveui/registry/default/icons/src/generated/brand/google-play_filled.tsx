/** Auto-generated - Do not edit */
'use client';
import React from 'react';

export interface GooglePlayFilledProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  strokeWidth?: number;
}

export const GooglePlayFilled = React.forwardRef<SVGSVGElement, GooglePlayFilledProps>(
  ({ size = 24, className = '', strokeWidth = 1, ...props }, ref) => (
    <svg 
      ref={ref}
      width={size}
      height={size} 
      viewBox="0 0 24 24"
      fill="currentColor"
      stroke="none"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...(strokeWidth !== undefined ? { strokeWidth } : {})}
      {...props}
    >
      <path d="M21.4 12.95L18.62 14.34C18.43 14.44 18.2 14.4 18.04 14.25L16.17 12.38C15.97 12.18 15.97 11.87 16.17 11.67L18.07 9.77002C18.22 9.62002 18.46 9.58002 18.65 9.68002L21.4 11.06C22.19 11.44 22.19 12.56 21.4 12.95Z" fill="currentColor"/>
<path d="M16.3 8.48996C16.61 8.63996 16.67 9.04998 16.43 9.28998L14.76 10.96L14.05 11.67C13.85 11.87 13.54 11.87 13.34 11.67L6.19997 4.52997C5.79997 4.12997 6.26997 3.47998 6.77997 3.72998L16.3 8.48996Z" fill="currentColor"/>
<path d="M2 20.1501V3.48008C2 3.04008 2.53 2.81007 2.85 3.12007L11.62 11.6301C11.83 11.8301 11.82 12.1601 11.61 12.3601L2.84 20.5201C2.52 20.8201 2 20.5901 2 20.1501Z" fill="currentColor"/>
<path d="M16.2701 15.52L6.88007 20.22C6.37007 20.47 5.90007 19.82 6.30007 19.42L13.3501 12.37C13.5501 12.17 13.8601 12.17 14.0601 12.37L16.4101 14.72C16.6401 14.96 16.5701 15.37 16.2701 15.52Z" fill="currentColor"/>
    </svg>
  )
);
GooglePlayFilled.displayName = "GooglePlayFilled";
export const GooglePlayFilledMetadata = { 
  id: "google-play_filled", 
  baseId: "google-play", 
  variant: "filled", 
  name: "Google Play", 
  category: "brand", 
  tags: [], 
  viewBox: "0 0 24 24" 
} as const;

export default GooglePlayFilled;
