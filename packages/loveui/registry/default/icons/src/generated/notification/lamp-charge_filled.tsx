/** Auto-generated - Do not edit */
'use client';
import React from 'react';

export interface LampChargeFilledProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
  strokeWidth?: number;
}

export const LampChargeFilled = React.forwardRef<SVGSVGElement, LampChargeFilledProps>(
  ({ size = 24, className = '', strokeWidth = 1, ...props }, ref) => (
    <svg 
      ref={ref}
      width={size}
      height={size} 
      viewBox="0 0 24 24"
      fill="currentColor"
      stroke="none"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      {...(strokeWidth !== undefined ? { strokeWidth } : {})}
      {...props}
    >
      <path d="M15.26 21.9999C15.2 21.9999 15.13 21.9899 15.07 21.9699C13.06 21.3999 10.95 21.3999 8.94003 21.9699C8.57003 22.0699 8.18003 21.8599 8.08003 21.4899C7.97003 21.1199 8.19003 20.7299 8.56003 20.6299C10.82 19.9899 13.2 19.9899 15.46 20.6299C15.83 20.7399 16.05 21.1199 15.94 21.4899C15.84 21.7999 15.56 21.9999 15.26 21.9999Z" fill="currentColor"/>
<path d="M19.21 6.36001C18.17 4.26001 16.16 2.71001 13.83 2.20001C11.39 1.66001 8.88997 2.24001 6.97997 3.78001C5.05997 5.31001 3.96997 7.60001 3.96997 10.05C3.96997 12.64 5.51997 15.35 7.85997 16.92V17.75C7.84997 18.03 7.83997 18.46 8.17997 18.81C8.52997 19.17 9.04997 19.21 9.45997 19.21H14.59C15.13 19.21 15.54 19.06 15.82 18.78C16.2 18.39 16.19 17.89 16.18 17.62V16.92C19.28 14.83 21.23 10.42 19.21 6.36001ZM13.72 11.62L12.65 13.48C12.51 13.72 12.26 13.86 12 13.86C11.87 13.86 11.74 13.83 11.63 13.76C11.27 13.55 11.15 13.09 11.35 12.74L12.2 11.26H11.36C10.86 11.26 10.45 11.04 10.23 10.67C10.01 10.29 10.03 9.83001 10.28 9.39001L11.35 7.53001C11.56 7.17001 12.02 7.05001 12.37 7.25001C12.73 7.46001 12.85 7.92001 12.65 8.27001L11.8 9.75001H12.64C13.14 9.75001 13.55 9.97001 13.77 10.34C13.99 10.72 13.97 11.19 13.72 11.62Z" fill="currentColor"/>
    </svg>
  )
);
LampChargeFilled.displayName = "LampChargeFilled";
export const LampChargeFilledMetadata = { 
  id: "lamp-charge_filled", 
  baseId: "lamp-charge", 
  variant: "filled", 
  name: "Lamp Charge", 
  category: "notification", 
  tags: [], 
  viewBox: "0 0 24 24" 
} as const;

export default LampChargeFilled;
