...
describe("searchRegistries", () => {
  it("should fetch and return registries in flat format", async () => {
    ...
    mockGetRegistry.mockImplementation(async (name: string) => {
      if (name === "@love-ui" || name === "@love-ui/registry") {
        return {
          name: "loveui",
    ...
    const results = await searchRegistries(["@love-ui", "@custom"])
      ...
      items: [
        {
          ...
          description: "A button component",
          registry: "@love-ui",
          addCommandArgument: "@love-ui/button",
        },
        {
          name: "card",
          type: "registry:ui",
          description: "A card component",
          registry: "@love-ui",
          addCommandArgument: "@love-ui/card",
        },
        {
          name: "header",
          type: "registry:component",
  ...
  it("should apply search filter when query is provided", async () => {
    ...
    const results = await searchRegistries(["@love-ui"], { query: "button" })

    expect(results.items).toHaveLength(1)
    expect(results.items[0].name).toBe("button")
    expect(results.items[0].registry).toBe("@love-ui")
    expect(results.items[0].addCommandArgument).toBe("@love-ui/button")
  ...
  it("should handle mixed registry names and URLs", async () => {
    ...
    mockGetRegistry.mockImplementation(async (nameOrUrl: string) => {
      if (nameOrUrl === "@love-ui" || nameOrUrl === "@love-ui/registry") {
        return {
          name: "loveui",
          homepage: "https://www.loveui.dev",
          items: [
    ...
    // Should find the button from @love-ui
    expect(results.items).toHaveLength(1)
    expect(results.items[0]).toMatchObject({
      name: "button",
